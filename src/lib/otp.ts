...
// Signup phone-OTP. Codes are stored HASHED in traderslink_otps (never plaintext),
...
export async function sendSignupOtp(
  ...
  const existing = (await sql`
    SELECT last_sent_at FROM traderslink_otps WHERE phone = ${phone} LIMIT 1
  ...
  await sql`
    INSERT INTO traderslink_otps (phone, code_hash, expires_at, attempts, last_sent_at)
...
export async function verifySignupOtp(
  ...
  const rows = (await sql`
    SELECT code_hash, expires_at, attempts FROM traderslink_otps WHERE phone = ${phone} LIMIT 1
  ...
  if (hashCode(phone, clean) !== otp.code_hash) {
    await sql`UPDATE traderslink_otps SET attempts = attempts + 1 WHERE phone = ${phone}`;
    return { ok: false, error: "Incorrect code. Try again." };
  }

  // Correct — consume it so it can't be reused.
  await sql`DELETE FROM traderslink_otps WHERE phone = ${phone}`;
  return { ok: true };
}
